...
正式版剩余验收及 1.0.0 开发交接见 [版本路线图](docs/ROADMAP.md)。计划与实际验收结果分别记录，未完成项不代表已提供的功能。
...
构建得到 `dist/markbridge-0.7.0-rc.2.zip`。仓库保留可读源码，构建产物与第三方文件不参与手工格式化。完整运行测试需要按安装指南准备与目标 WordPress 匹配的私有运行环境。
