...
window.addEventListener(
  ...
  (event) => {
    ...
    if (src && new URL(src, location.href).pathname.endsWith('/vendor/mathjax-4.1.3/tex-svg.js')) {
...
window.addEventListener('message', (event) => {
  ...
  if (m.action === 'hello') {
    ...
    MathJax.startup.promise
      .then(() => {
        if (typeof MathJax.tex2svgPromise !== 'function') throw Error('公式接口未就绪');
        mbbReady = true;
        reply({ action: 'ready' });
      })
      .catch((e) => {
  ...
  if (m.action === 'render' && mbbReady && Number.isSafeInteger(m.id))
    mbbRender(m.tex, !!m.display).then(
      (html) => reply({ action: 'result', id: m.id, html }),
